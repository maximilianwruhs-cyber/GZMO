...
export OUT APPLY HOST GZMO_BIN SPARK_RAW ROOT DATA
python3 - <<'PY'
...
applied = []
apply_error = None
if apply and takeaways and not dual_writer:
    ...
    if p.returncode != 0:
        ...
        # One combined takeaway (session close accepts one --takeaway string)
        combined = " | ".join(takeaways[:3])
        cmd = (
    ...
    apply_error = "no_candidates"

advice = "serendipity_promote_dry_run_ok"
if dual_writer:
    advice = "serendipity_promote_hold — dual_writer active"
elif not takeaways:
    advice = "serendipity_promote_hold — no spark candidates (run spark / digest first)"
elif apply and applied:
    advice = "serendipity_promote_applied — takeaway enqueued on living host"
...
payload = {
    ...
    "next": [
        ...
        "Review takeaways in promote artifact",
