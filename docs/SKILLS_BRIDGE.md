...
Würfel / cascade / feat attractor stack are **Unpark Wave 2** — see [PANTHEON_FEAT_RELAND.md](./PANTHEON_FEAT_RELAND.md). On `main` today: **Slice A full** — TOML dice corpus, nested wild-magic cascade via `dispatch`, Card Forge + story (`attractor_common` / `generative`). Still deferred: `pkm_forge`, daemon `dice_loop`, Slice C chaos IPC. Ghost `DICE_MASTER_*` masters never existed — do not invent them.
