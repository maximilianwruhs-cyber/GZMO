...
Würfel / cascade / feat attractor stack are **Unpark Wave 2** — see [PANTHEON_FEAT_RELAND.md](./PANTHEON_FEAT_RELAND.md). On `main` today: Slice **A.0** (TOML dice corpus) + **A.1** (plan-only wild-magic suggestion after `/dice`). Nested cascade execute + Card Forge still need `attractor_common` / `dispatch`. Ghost `DICE_MASTER_*` masters never existed — do not invent them.
