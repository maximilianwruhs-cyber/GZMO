...
|-------|------------|-------|
| **GZMO-core ObolusGate** | Token/`E_total` + ctx% gates for T0–T2 autonomy | This repo — [OBOLUS_GOVERNANCE.md](./OBOLUS_GOVERNANCE.md) |
| **Obolus product** | Local model economics: `z = quality / (joules × price_factor)`, MCP recommend | Sibling `Obolus/` — public stranger path |
| **Obolus Arena lab** | DNA → dense SFT/DPO → ratchet → organs → overnight z loops | Sibling `obolus-arena/` — **not** CT101 living brain |
...
```
Arena proves an organ overnight
  → only proven defaults
  → Obolus MCP recommend (display / route advice)
  → never blocks CT101 distill/dream unless operator gates say so
```
