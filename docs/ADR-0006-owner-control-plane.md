...
2. **CLI (`gzmo memory *`) and MCP are clients.** They prefer the socket when it is live. `--offline` or `GZMO_CONTROL_PLANE=0` forces in-process `PlatformMemory` (telescope / inspect — not a lite SKU; [ADR-0007](./ADR-0007-one-product-living.md)).
...
4. **Living hard-fail.** Vault under `/opt/gzmo` with a dead socket refuses in-process open unless `--offline` (inspect only; refused while the owner is up) or `GZMO_CONTROL_PLANE=0`. `~/.gzmo` and telescope lab vaults stay in-process (scratch, not a lite SKU; [ADR-0007](./ADR-0007-one-product-living.md)).
