...
| `/stabilize` + skill | `feedback.rs`, `chat.rs`, `skills/skill_stabilize.sh` | Δρ = −1.0 default |
| Daemon PulseLoop + Synapse | `daemon_cmd.rs`, `chaos_bootstrap.rs` | `chaos.rho_telemetry` every 15 ticks |
| Edge TS parity | `survey_edge-node` chaos-engine + gzmo-daemon | decay, tanh, EMA, stabilize event |

**Tests:** 18 `gzmo-chaos` unit tests passing.
...
| `TriggerCondition::BreathPhaseEnter` | No consumer today | Add after breath detector choice is frozen |
