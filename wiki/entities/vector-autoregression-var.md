---
type: entity
title: Vector Autoregression (VAR)
created: 2026-06-08
updated: 2026-06-08
sources: 1
tags: []
status: draft
gzmo_synthetic: true
---

# Vector Autoregression (VAR)

Type: CONCEPT

## From [[drive-research-cache-optimization-with-ai-chaos-theory|drive-research-cache-optimization-with-ai-chaos-theory]] (2026-06-08)
- A linear stochastic model used in traditional financial market modeling.
- Fails to capture high-frequency microstructural dynamics of modern limit order books.
