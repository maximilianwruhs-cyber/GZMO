---
type: entity
title: PaddleOCR
created: 2026-06-08
updated: 2026-06-10
sources: 3
tags: []
status: draft
gzmo_synthetic: true
---



# PaddleOCR

Type: TOOL

## From [[local-first-rag-architecting-sovereign-ai-with-li|local-first-rag-architecting-sovereign-ai-with-li]] (2026-06-08)
- Highly effective for complex Chinese layouts.
- Can be plugged into LiteParse via a standardized HTTP API specification.

## From [[drive-research-pdf-text-vs-scan-detection-heuristics-micro03|drive-research-pdf-text-vs-scan-detection-heuristics-micro03]] (2026-06-09)
- Mentioned as a potential heavy OCR engine.

## From [[drive-research-pdf-text-vs-scan-detection-heuristics-micro04|drive-research-pdf-text-vs-scan-detection-heuristics-micro04]] (2026-06-10)
- An OCR toolkit that bridges the gap between images/PDFs and LLMs.
- Supports 100+ languages.
