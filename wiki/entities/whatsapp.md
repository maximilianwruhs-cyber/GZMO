---
type: entity
title: WhatsApp
created: 2026-06-08
updated: 2026-06-10
sources: 23
tags: []
status: draft
gzmo_synthetic: true
---























# WhatsApp

Type: SYSTEM

## From [openclaw-deep-research-part2](/entities/openclaw-deep-research-part2.md) (2026-06-08)
- OpenClaw's functionality is accessed via a chatbot within a messaging service, such as WhatsApp.
- WhatsApp is a messaging platform.

## From [openclaw-deep-research-part4](/entities/openclaw-deep-research-part4.md) (2026-06-08)
- OpenClaw has native integrations into it.
- OpenClaw agents can operate natively inside it.

## From [openclaw-deep-research-part6](/entities/openclaw-deep-research-part6.md) (2026-06-08)
- A chat platform that OpenClaw can operate across.
- Users can interact with their assistant through this platform.
- One of the platforms supported by OpenClaw Channel Adapters.
- Initial launch of Clawdbot was a "WhatsApp Relay" project.

## From [openclaw-autonomous-ai-agents-in-financial-operat](/entities/openclaw-autonomous-ai-agents-in-financial-operat.md) (2026-06-08)
- Messaging application used to interact with the OpenClaw agent.
- Used for displaying Command Cards for HITL approval.

## From [drive-research-hermes-compression-and-bol-architecture](/entities/drive-research-hermes-compression-and-bol-architecture.md) (2026-06-08)
- Messaging platform where Hermes can operate.
- Session keys are correlated to internal active session IDs.

## From [drive-research-soul](/entities/drive-research-soul.md) (2026-06-08)
- One of the User's messaging channels.
- Used for autonomous communication by GZMO.

## From [openclaw-deep-research-part1-micro03](/entities/openclaw-deep-research-part1-micro03.md) (2026-06-09)
- A channel that can be configured in OpenClaw.
- Supports tap-to-mention.
- Can be configured for group chat mention gating.

## From [openclaw-deep-research-part5-micro03](/entities/openclaw-deep-research-part5-micro03.md) (2026-06-09)
- Users can talk to OpenClaw on WhatsApp.
- OpenClaw works well on WhatsApp.

## From [the-openclaw-architecture-and-tri-circuit-autonomo-part1-micro06](/entities/the-openclaw-architecture-and-tri-circuit-autonomo-part1-micro06.md) (2026-06-09)
- Messaging platform managed by OpenClaw Gateway.
- Uses Baileys adapter for inbound payload translation.

## From [openclaw-deep-research-part1-micro05](/entities/openclaw-deep-research-part1-micro05.md) (2026-06-10)
- A configured channel for heartbeat delivery.

## From [openclaw-deep-research-part1-micro06](/entities/openclaw-deep-research-part1-micro06.md) (2026-06-10)
- A messaging platform that connects to the OpenClaw Channel Layer via Baileys.

## From [openclaw-deep-research-part1-micro07](/entities/openclaw-deep-research-part1-micro07.md) (2026-06-10)
- A communication channel that can be connected to OpenClaw
- Supports dmPolicy and groupPolicy configurations

## From [openclaw-deep-research-part10-micro08](/entities/openclaw-deep-research-part10-micro08.md) (2026-06-10)
- External messaging surface that should not receive streaming/partial replies

## From [openclaw-deep-research-part5-micro02](/entities/openclaw-deep-research-part5-micro02.md) (2026-06-10)
- Used as a platform for personal AI assistants.

## From [openclaw-deep-research-part5-micro05](/entities/openclaw-deep-research-part5-micro05.md) (2026-06-10)
- A messaging platform supported by OpenClaw's native integration

## From [openclaw-deep-research-part5-micro06](/entities/openclaw-deep-research-part5-micro06.md) (2026-06-10)
- A messaging platform supported by OpenClaw.

## From [openclaw-deep-research-part5-micro07](/entities/openclaw-deep-research-part5-micro07.md) (2026-06-10)
- A messaging platform that OpenClaw can connect to.

## From [openclaw-deep-research-part5-micro08](/entities/openclaw-deep-research-part5-micro08.md) (2026-06-10)
- A messaging platform supported by OpenClaw.

## From [openclaw-deep-research-part9-micro01](/entities/openclaw-deep-research-part9-micro01.md) (2026-06-10)
- A messaging surface supported via Baileys (web protocol).

## From [openclaw-deep-research-part9-micro03](/entities/openclaw-deep-research-part9-micro03.md) (2026-06-10)
- Messaging app that pairs via QR code

## From [openclaw-deep-research-part9-micro04](/entities/openclaw-deep-research-part9-micro04.md) (2026-06-10)
- A communication channel that can be connected to OpenClaw
- Supports dmPolicy settings like 'pairing'

## From [openclaw-deep-research-part9-micro05](/entities/openclaw-deep-research-part9-micro05.md) (2026-06-10)
- A supported communication channel

## From [openclaw-part1-micro03](/entities/openclaw-part1-micro03.md) (2026-06-10)
- Supports sophisticated routing and access control via the channels.whatsapp.allowFrom array.
- Features reactionLevel guidance for autonomous emoji reactions.
