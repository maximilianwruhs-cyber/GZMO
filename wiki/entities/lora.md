---
type: entity
title: LoRA
created: 2026-06-09
updated: 2026-06-09
sources: 2
tags: []
status: draft
gzmo_synthetic: true
---


# LoRA

Type: TOOL

## From [[architectures-and-optimizations-for-speculative-de-micro02|architectures-and-optimizations-for-speculative-de-micro02]] (2026-06-09)
- Used to augment highly optimized single systems for comparison.

## From [[prompt-agent-engineering-part4-micro01|prompt-agent-engineering-part4-micro01]] (2026-06-09)
- Low-Rank Adaptation.
- Used for over-the-air model updates to edge nodes.
- MB-sized updates that teach new skills to Subagents.
