---
type: entity
title: Llama 4 Maverick
created: 2026-06-09
updated: 2026-06-10
sources: 4
tags: []
status: draft
gzmo_synthetic: true
---




# Llama 4 Maverick

Type: SYSTEM

## From [[drive-research-linux-gaming-and-ai-build-guide-micro01|drive-research-linux-gaming-and-ai-build-guide-micro01]] (2026-06-09)
- A MoE model with 400B total parameters and 17B active.
- Part of the Llama 4 family.
- Drastically changes hardware requirements for deployment.

## From [[drive-research-linux-gaming-and-ai-build-guide-micro05|drive-research-linux-gaming-and-ai-build-guide-micro05]] (2026-06-09)
- Mixture-of-Experts (MoE) model.
- 400B total parameters, 17B active.

## From [[drive-research-llm-inference-engine-audit-2026-micro02|drive-research-llm-inference-engine-audit-2026-micro02]] (2026-06-09)
- Model that breaks the 1000 TPS/User Barrier with Blackwell.

## From [[the-2026-linux-workstation-micro02|the-2026-linux-workstation-micro02]] (2026-06-10)
- 400B total parameters, 17B active
- Model family introduced by Meta in 2026
- Utilizes Mixture-of-Experts (MoE) architectures
