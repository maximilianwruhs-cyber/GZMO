---
type: entity
title: Hugging Face
created: 2026-06-08
updated: 2026-06-10
sources: 6
tags: []
status: draft
gzmo_synthetic: true
---






# Hugging Face

Type: ORGANIZATION

## From [drive-research-free-lob-data-for-chaos-research](/entities/drive-research-free-lob-data-for-chaos-research.md) (2026-06-08)
- A public repository mirror for LOBSTER data is accessible via Hugging Face.
- Imbalance Labs Normalized DEX L2 Dataset is hosted on Hugging Face.
- Hosts datasets.
- Has a category for 'quant' datasets.

## From [drive-research-hidden-mode-technical-analysis-and-configurati](/entities/drive-research-hidden-mode-technical-analysis-and-configurati.md) (2026-06-08)
- Provides resources like blog posts and model repositories related to MoE models and mergekit.
- Hosts models like mlabonne/Beyonder-4x7B-v3 and openchat/openchat-3.5-1210.

## From [architectures-and-optimizations-for-speculative-de-micro03](/entities/architectures-and-optimizations-for-speculative-de-micro03.md) (2026-06-09)
- Provides a paper page for Recursive Multi-Agent Systems.
- Hosts daily papers.

## From [the-architecture-of-speculative-decoding-and-infer-part1-micro05](/entities/the-architecture-of-speculative-decoding-and-infer-part1-micro05.md) (2026-06-09)
- A framework mentioned for implementing TurboQuant and speculative decoding systems.

## From [openclaw-deep-research-part8-micro02](/entities/openclaw-deep-research-part8-micro02.md) (2026-06-10)
- Provides model hosting integrations

## From [openclaw-deep-research-part8-micro05](/entities/openclaw-deep-research-part8-micro05.md) (2026-06-10)
- Offers professional services for AutoGen
