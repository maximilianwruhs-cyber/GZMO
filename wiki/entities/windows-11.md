---
type: entity
title: Windows 11
created: 2026-06-08
updated: 2026-06-09
sources: 3
tags: []
status: draft
gzmo_synthetic: true
---



# Windows 11

Type: SYSTEM

## From [[architecting-the-minimalist-linux-desktop-a-compa-part2|architecting-the-minimalist-linux-desktop-a-compa-part2]] (2026-06-08)
- Mandates strict adherence to UEFI and Secure Boot protocols.
- Will diminish the necessity for MBR-based legacy boot mechanisms.
- Represents a move away from MBR-based legacy boot.

## From [[drive-research-linux-gaming-and-ai-build-guide-micro03|drive-research-linux-gaming-and-ai-build-guide-micro03]] (2026-06-09)
- Matched by SteamOS at 4K Ultra on AMD for Linux Gaming 2026

## From [[drive-research-linux-gaming-and-ai-build-guide-micro07|drive-research-linux-gaming-and-ai-build-guide-micro07]] (2026-06-09)
- SteamOS matches Windows 11 at 4K Ultra on AMD
