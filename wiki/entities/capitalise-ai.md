---
type: entity
title: Capitalise.ai
created: 2026-06-09
updated: 2026-06-10
sources: 6
tags: []
status: draft
gzmo_synthetic: true
---






# Capitalise.ai

Type: TOOL

## From [drive-research-setting-up-autonomous-trading-agents-micro01](/entities/drive-research-setting-up-autonomous-trading-agents-micro01.md) (2026-06-09)
- A specialized no-code trading automation platform.
- Enables traders to construct algorithms using plain English or IFTTT rule builders.
- Monitors market data, technical indicators, and news events 24/7.
- Restricts execution to spot instruments and crypto futures.
- Discontinued FIX connection support for Interactive Brokers in early 2026.

## From [drive-research-setting-up-autonomous-trading-agents-micro02](/entities/drive-research-setting-up-autonomous-trading-agents-micro02.md) (2026-06-09)
- Consumer-friendly platform.
- Ceased supporting IBKR's FIX connections.
- Offers immediate accessibility but severely restricts strategic depth.

## From [drive-research-setting-up-autonomous-trading-agents-micro05](/entities/drive-research-setting-up-autonomous-trading-agents-micro05.md) (2026-06-09)
- A pure, visually driven no-code platform.
- Offers immediate accessibility but restricts strategic depth.
- Introduces substantial vendor lock-in risks.
- Abruptly terminated Interactive Brokers support.

## From [the-architects-handbook-for-autonomous-agentic-tr-micro02](/entities/the-architects-handbook-for-autonomous-agentic-tr-micro02.md) (2026-06-09)
- A specialized no-code trading automation platform.
- Enables traders to construct algorithms using plain English or IFTTT rule builders.
- Restricts execution to spot instruments and crypto futures.
- Specialized no-code trading automation platform.

## From [the-architects-handbook-for-autonomous-agentic-tr-micro03](/entities/the-architects-handbook-for-autonomous-agentic-tr-micro03.md) (2026-06-09)
- Ceased supporting IBKR's FIX connections.
- A consumer-friendly platform.
- Offers immediate accessibility but severely restricts strategic depth.

## From [drive-research-setting-up-autonomous-trading-agents-micro04](/entities/drive-research-setting-up-autonomous-trading-agents-micro04.md) (2026-06-10)
- Enables traders to construct algorithms using plain English or IFTTT rule builders.
- Restricts execution to spot instruments and cryptocurrency futures.
- Discontinued FIX connection support for Interactive Brokers in early 2026.
