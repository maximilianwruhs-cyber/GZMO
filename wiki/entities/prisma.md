---
type: entity
title: PRISMA
created: 2026-06-08
updated: 2026-06-09
sources: 4
tags: []
status: draft
gzmo_synthetic: true
---




# PRISMA

Type: FRAMEWORK

## From [the-architecture-of-scientific-inquiry-and-academi](/entities/the-architecture-of-scientific-inquiry-and-academi.md) (2026-06-08)
- Preferred Reporting Items for Systematic Reviews and Meta-Analyses.
- Designed to improve the reporting of evidence synthesis.
- Includes a checklist, explanation document, and flow diagram; has extensions for various review types.

## From [drive-research-die-architektur-der-wissenschaftlichen-forschung](/entities/drive-research-die-architektur-der-wissenschaftlichen-forschung.md) (2026-06-08)
- Preferred Reporting Items for Systematic Reviews and Meta-Analyses.
- Designed to improve the reporting of evidence syntheses.
- Includes a 27-point checklist, an explanation document, and a flowchart.

## From [drive-research-scientific-writing-and-publication-process-micro01](/entities/drive-research-scientific-writing-and-publication-process-micro01.md) (2026-06-09)
- It is a reporting guideline for systematic reviews and meta-analyses.
- It includes a 27-item checklist, an explanation document, and a flow diagram.
- Extensions cover Protocols (PRISMA-P), Individual Participant Data (PRISMA-IPD), Network Meta-analysis (PRISMA-NMA), Diagnostic Test Accuracy (PRISMA-DTA), and Scoping Reviews (PRISMA-ScR).

## From [drive-research-scientific-writing-and-publication-process-micro04](/entities/drive-research-scientific-writing-and-publication-process-micro04.md) (2026-06-09)
- guideline for reporting
- mentioned in the context of research reporting
- used in scientific writing
