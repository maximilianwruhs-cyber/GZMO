---
type: entity
title: DLSS 4.5
created: 2026-06-09
updated: 2026-06-09
sources: 2
tags: []
status: draft
gzmo_synthetic: true
---


# DLSS 4.5

Type: TOOL

## From [drive-research-linux-gaming-and-ai-build-guide-micro01](/entities/drive-research-linux-gaming-and-ai-build-guide-micro01.md) (2026-06-09)
- Introduces Dynamic Multi Frame Generation.
- Allows frame rates to be pushed exponentially beyond raw hardware limits.

## From [drive-research-linux-gaming-and-ai-build-guide-micro04](/entities/drive-research-linux-gaming-and-ai-build-guide-micro04.md) (2026-06-09)
- Introduces Dynamic Multi Frame Generation.
- Allows up to a 6x multiplier to push frame rates exponentially beyond raw hardware limits.
