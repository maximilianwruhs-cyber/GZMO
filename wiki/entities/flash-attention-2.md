---
type: entity
title: Flash Attention 2
created: 2026-06-09
updated: 2026-06-09
sources: 2
tags: []
status: draft
gzmo_synthetic: true
---


# Flash Attention 2

Type: TOOL

## From [drive-research-linux-gaming-and-ai-build-guide-micro01](/entities/drive-research-linux-gaming-and-ai-build-guide-micro01.md) (2026-06-09)
- Older equivalents are used on ROCm via MIOpen.
- Represents an earlier version of FlashAttention.

## From [drive-research-linux-gaming-and-ai-build-guide-micro04](/entities/drive-research-linux-gaming-and-ai-build-guide-micro04.md) (2026-06-09)
- Older equivalents are used by ROCm via MIOpen.
- FlashAttention 3 is unavailable on ROCm.
