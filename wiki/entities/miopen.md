---
type: entity
title: MIOpen
created: 2026-06-09
updated: 2026-06-09
sources: 2
tags: []
status: draft
gzmo_synthetic: true
---


# MIOpen

Type: TOOL

## From [[drive-research-linux-gaming-and-ai-build-guide-micro01|drive-research-linux-gaming-and-ai-build-guide-micro01]] (2026-06-09)
- ROCm relies on MIOpen for Flash Attention 2 equivalents.
- AMD's library for deep learning primitives.

## From [[drive-research-linux-gaming-and-ai-build-guide-micro04|drive-research-linux-gaming-and-ai-build-guide-micro04]] (2026-06-09)
- ROCm relies on MIOpen for older Flash Attention 2 equivalents.
- AMD's deep learning library.
