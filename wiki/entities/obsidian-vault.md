...
- Stores knowledge base as local Markdown (.md) files.
- Seamlessly meshes with local AI ecosystem.
- Can be read by AI with TurboQuant.
- Acts as Long-Term Memory and Architect's Blueprint.
- Used for 'Docs-Driven Development'.
- Used for Agentic Task Management.
- Enables 'Reverse Workflow' for automated documentation.
- Can be indexed for Continue.dev RAG.
- Requires MCP to give Roo Code 'Hands'.
- Stores 'Personas' and 'Agentic Skills' for Roo Code.
- Can store OpenAPI specifications.
- Can store Google Docs requirements via MCP.
- Acts as the FOSS Product Manager.
- Used as the Architect's Brain.
- Should not be installed via Ubuntu Snap store.
- Vault is used as a local, searchable database of engineering knowledge.
- Used as a 'brain' for storing blueprints and ideas.
- Files are used for communication between AI agents.
- The 'Human-in-the-Loop Obsidian Workspace' is a recommended setup.
- Installed natively.
