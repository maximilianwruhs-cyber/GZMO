---
type: entity
title: Moltbot
created: 2026-06-08
updated: 2026-06-10
sources: 10
tags: []
status: draft
gzmo_synthetic: true
---










# Moltbot

Type: SYSTEM

## From [[openclaw-part2|openclaw-part2]] (2026-06-08)
- A name OpenClaw was briefly known as during its rapid iterations.
- First rebrand name for Clawdbot on Jan 27, 2026.
- Renamed following a trademark notice from Anthropic regarding the name 'Clawd'.
- Renamed to OpenClaw to address pronunciation issues.

## From [[openclaw-deep-research-part2|openclaw-deep-research-part2]] (2026-06-08)
- Moltbot was a renamed version of Clawdbot on January 27, 2026.
- It was renamed to OpenClaw three days later.
- The renaming followed trademark complaints by Anthropic.

## From [[openclaw-deep-research-part6|openclaw-deep-research-part6]] (2026-06-08)
- Previous name for OpenClaw.
- Caused confusion in package names, docs, and endpoint prefixes during rename history.
- Renamed from Clawdbot following a trademark notice.
- OpenClaw was formerly known as Moltbot.
- The previous name for OpenClaw.
- The name was difficult to pronounce and did not resonate well with the community.

## From [[openclaw-deep-research-part12|openclaw-deep-research-part12]] (2026-06-08)
- An alternative name for OpenClaw.

## From [[openclaw-autonomous-ai-agents-in-financial-operat|openclaw-autonomous-ai-agents-in-financial-operat]] (2026-06-08)
- Briefly used as a name for OpenClaw before settling on OpenClaw.

## From [[openclaw-deep-research-part5-micro03|openclaw-deep-research-part5-micro03]] (2026-06-09)
- Formerly known as OpenClaw.

## From [[openclaw-deep-research-part5-micro04|openclaw-deep-research-part5-micro04]] (2026-06-09)
- Rebranded name for Clawdbot.
- Finalized identity as OpenClaw on January 30, 2026.

## From [[the-openclaw-architecture-and-tri-circuit-autonomo-part1-micro01|the-openclaw-architecture-and-tri-circuit-autonomo-part1-micro01]] (2026-06-09)
- Briefly renamed OpenClaw due to trademark disputes.
- Renamed before settling on OpenClaw in January 2026.

## From [[the-openclaw-architecture-and-tri-circuit-autonomo-part1-micro06|the-openclaw-architecture-and-tri-circuit-autonomo-part1-micro06]] (2026-06-09)
- Briefly adopted name for OpenClaw.
- Preceded the stabilization as OpenClaw.

## From [[openclaw-deep-research-part7-micro06|openclaw-deep-research-part7-micro06]] (2026-06-10)
- Former name of OpenClaw
