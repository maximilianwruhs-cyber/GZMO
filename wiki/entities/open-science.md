---
type: entity
title: Open Science
created: 2026-06-08
updated: 2026-06-09
sources: 3
tags: []
status: draft
gzmo_synthetic: true
---



# Open Science

Type: CONCEPT

## From [[the-architecture-of-scientific-inquiry-and-academi|the-architecture-of-scientific-inquiry-and-academi]] (2026-06-08)
- Demands researchers make underlying data and analytical pipelines accessible.
- Requires implementation of data management plans and computational reproducibility.
- A paradigm shift in scientific communication.

## From [[drive-research-die-architektur-der-wissenschaftlichen-forschung|drive-research-die-architektur-der-wissenschaftlichen-forschung]] (2026-06-08)
- A paradigm shift in research.
- Requires researchers to share underlying data and analysis pipelines.
- Promotes transparency and collaboration.

## From [[drive-research-scientific-writing-and-publication-process-micro01|drive-research-scientific-writing-and-publication-process-micro01]] (2026-06-09)
- It demands that researchers make underlying data and analytical pipelines accessible.
- It requires the implementation of sophisticated data management plans, computational reproducibility protocols, and advanced project management strategies.
