---
type: entity
title: Claude Opus 4.6
created: 2026-06-09
updated: 2026-06-10
sources: 7
tags: []
status: draft
gzmo_synthetic: true
---









# Claude Opus 4.6

Type: MODEL

## From [[drive-research-agentic-reverse-engineering-state-and-future-micro02|drive-research-agentic-reverse-engineering-state-and-future-micro02]] (2026-06-09)
- An earlier frontier model that struggled with autonomous exploit development.
- Had a near-zero success rate in autonomous exploit development on complex benchmarks.

## From [[drive-research-agentic-reverse-engineering-state-and-future1-micro02|drive-research-agentic-reverse-engineering-state-and-future1-micro02]] (2026-06-09)
- An earlier frontier model that struggled with autonomous exploit development.
- Demonstrated a near-zero success rate in autonomous exploit development.

## From [[the-evolution-of-artificial-intelligence-evaluatio-micro02|the-evolution-of-artificial-intelligence-evaluatio-micro02]] (2026-06-09)
- Routinely scores between ninety-four and ninety-nine percent on GSM8K.
- Scores eighty point eight percent on SWE-bench Verified.

## From [[openclaw-deep-research-part9-micro02|openclaw-deep-research-part9-micro02]] (2026-06-10)
- Model used by the Loki instance via AWS Bedrock

## From [[openclaw-deep-research-part9-micro03|openclaw-deep-research-part9-micro03]] (2026-06-10)
- Model for heavy reasoning tasks

## From [[openclaw-deep-research-part9-micro04|openclaw-deep-research-part9-micro04]] (2026-06-10)
- An AI model provided by Anthropic
- Used for complex reasoning tasks

## From [[the-dawn-of-agentic-software-reverse-engineering-micro02|the-dawn-of-agentic-software-reverse-engineering-micro02]] (2026-06-10)
- Earlier frontier model with a near-zero success rate in autonomous exploit development.
