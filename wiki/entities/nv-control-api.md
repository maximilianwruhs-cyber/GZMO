---
type: entity
title: NV-CONTROL API
created: 2026-06-09
updated: 2026-06-09
sources: 2
tags: []
status: draft
gzmo_synthetic: true
---


# NV-CONTROL API

Type: TOOL

## From [[drive-research-linux-gaming-and-ai-build-guide-micro01|drive-research-linux-gaming-and-ai-build-guide-micro01]] (2026-06-09)
- The NVIDIA Settings control panel previously utilized this antiquated API.
- An older API for controlling NVIDIA graphics settings.

## From [[drive-research-linux-gaming-and-ai-build-guide-micro04|drive-research-linux-gaming-and-ai-build-guide-micro04]] (2026-06-09)
- The NVIDIA Settings control panel transitioned away from this antiquated API.
- An older NVIDIA control API.
