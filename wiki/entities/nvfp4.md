---
type: entity
title: NVFP4
created: 2026-06-08
updated: 2026-06-10
sources: 3
tags: []
status: draft
gzmo_synthetic: true
---



# NVFP4

Type: CONCEPT

## From [[drive-research-blackwell-sm120-gemm-optimization-guide|drive-research-blackwell-sm120-gemm-optimization-guide]] (2026-06-08)
- MoE
- Native FP4
- MXFP8 and NVFP4
- NVFP4 issue root cause
- NVFP4 Explained
- Gemma 4 NVFP4
- SM120 NVFP4 GEMM

## From [[aether-grid-micro03|aether-grid-micro03]] (2026-06-09)
- A feature that some nodes might not support.
- Workloads need to be quantized or offloaded if not supported.

## From [[drive-research-marlin-baseline-for-early-deployments-micro01|drive-research-marlin-baseline-for-early-deployments-micro01]] (2026-06-10)
- Narrow-precision format.
- Natively accelerated on SM100 via tcgen05.
