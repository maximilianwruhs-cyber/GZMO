---
type: entity
title: Mendeley
created: 2026-06-08
updated: 2026-06-10
sources: 6
tags: []
status: draft
gzmo_synthetic: true
---






# Mendeley

Type: TOOL

## From [the-architecture-of-scientific-inquiry-and-academi](/entities/the-architecture-of-scientific-inquiry-and-academi.md) (2026-06-08)
- Built for STEM researchers and labs.
- Features PDF management, highlighting, cloud sync, and the Mendeley Cite Word add-in.
- Tight PDF workflow with seamless integrations into Elsevier publisher databases.

## From [drive-research-die-architektur-der-wissenschaftlichen-forschung](/entities/drive-research-die-architektur-der-wissenschaftlichen-forschung.md) (2026-06-08)
- A reference management tool.
- Designed for STEM researchers and labs.
- Offers PDF management, highlighting, cloud synchronization, and the Mendeley Cite Word add-in.

## From [drive-research-research-project-initiation-guide](/entities/drive-research-research-project-initiation-guide.md) (2026-06-08)
- A reference manager owned by Elsevier.
- Combines reference management with social discovery features.
- Provides 2 GB of free cloud storage.
- Available on Windows, macOS, Linux, Web.
- Free with 2 GB cloud storage.
- Integrates with Microsoft Word and LibreOffice.

## From [drive-research-scientific-writing-and-publication-process-micro01](/entities/drive-research-scientific-writing-and-publication-process-micro01.md) (2026-06-09)
- It is a reference management software built for STEM researchers and labs.
- It features PDF management, highlighting, cloud sync, and the Mendeley Cite Word add-in.
- It has a free core with paid upgrades.

## From [optimizing-nvidia-blackwell-sm120-part2-micro07](/entities/optimizing-nvidia-blackwell-sm120-part2-micro07.md) (2026-06-10)
- Reference management tool owned by Elsevier
- Includes 'Mendeley Suggest' feature for paper discovery

## From [optimizing-nvidia-blackwell-sm120-part2-micro08](/entities/optimizing-nvidia-blackwell-sm120-part2-micro08.md) (2026-06-10)
- Citation management tool
