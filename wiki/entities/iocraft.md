---
type: entity
title: Iocraft
created: 2026-06-09
updated: 2026-06-09
sources: 2
tags: []
status: draft
gzmo_synthetic: true
---


# Iocraft

Type: TOOL

## From [drive-research-rust-tui-architecture-tech-stack1-micro01](/entities/drive-research-rust-tui-architecture-tech-stack1-micro01.md) (2026-06-09)
- A declarative, React-style framework.
- Familiar to developers transitioning from web development ecosystems.
- Has a very nascent ecosystem.
- Lacks the deep repository of complex third-party widgets available to Ratatui.

## From [drive-research-rust-tui-architecture-tech-stack1-micro04](/entities/drive-research-rust-tui-architecture-tech-stack1-micro04.md) (2026-06-09)
- Declarative, React-style architectural paradigm.
- Familiar to developers transitioning from web development ecosystems.
- Very nascent ecosystem.
