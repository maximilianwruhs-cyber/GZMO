---
type: entity
title: Open VSX Registry
created: 2026-06-08
updated: 2026-06-09
sources: 5
tags: []
status: draft
gzmo_synthetic: true
---







# Open VSX Registry

Type: SYSTEM

## From [obolus-vs-codium-extension-konzept-research-part2](/entities/obolus-vs-codium-extension-konzept-research-part2.md) (2026-06-08)
- VS Codium Kompatibilität

## From [building-a-private-local-ai-development-environmen-micro02](/entities/building-a-private-local-ai-development-environmen-micro02.md) (2026-06-09)
- Used by VSCodium.

## From [building-a-private-local-ai-development-environmen-micro03](/entities/building-a-private-local-ai-development-environmen-micro03.md) (2026-06-09)
- wird von VSCodium genutzt
- eliminiert Abhängigkeit von proprietären Microsoft-Ökosystemen

## From [building-a-private-local-ai-development-environmen-micro04](/entities/building-a-private-local-ai-development-environmen-micro04.md) (2026-06-09)
- Used by VSCodium instead of Microsoft’s proprietary extension store.
- Allows Continue and OpenClaw extensions to be fully supported and available natively.

## From [ultimate-local-ai-development-stack-for-vscodium-micro01](/entities/ultimate-local-ai-development-stack-for-vscodium-micro01.md) (2026-06-09)
- An open-source registry used by VSCodium.
- Allows installation of extensions like Continue and OpenClaw.
