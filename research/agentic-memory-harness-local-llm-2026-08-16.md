...
| T1 | Assertable retrieve before rank; prune superseded vectors | GPM / Temporal Validity | `filter_assertable_honeypot_ids` / `sync-vault-to-qdrant.py` | **in-tree** (overfetch Qdrant then SQLite-filter; payload stamps `is_latest` — search filter still off until living re-sync) |
...
`failure_cases` retrieve (M3), workflow skill pinning (H1), named night labels (M7), ACE playbook curation (L1), and assertable retrieve (T1, including Qdrant overfetch) are **in-tree**.
