...
> **learning-loop-A (#6):** **CLOSED** — see [LEARNING_LOOP_A.md](../docs/LEARNING_LOOP_A.md); mid-band thicken + unified `night_id` landed.
...
|--:|-----------|-----------|-------------------------------|------------|--:|--:|--:|-----------------|
| 1 | `forget-lint` | Purposeful forgetting of intermediate honeypots | `690cb295`, `26ad76d0`, `20811f83`; cascading/wisdom + honeypot-compiler | **matured** — frontier real apply on vault *copy* | 9 | 7 | 9 | little-tool CLI |
| 2 | `verify-gates` | Mid-pipeline Analyze/Retrieve/Reason gates | `ffd73b42`, `11d7748d`, `b960e95c`; tinyfolder product analysis | **matured** — frontier analyze+retrieve+reason | 8 | 6 | 8 | little-tool / assembly piece |
| 3 | `token-economy` | Reactive budget/route/compress/cache | `06c23921`, `5d774056`, `9c827e76`; agentic-token-economy micros | **matured** — `recommend --emit-toml` advisory | 8 | 5 | 8 | little-tool CLI |
| 4 | `tool-chain` | Close “Tools Are Leaves” — follow refs → auto-read | `2102986b`, `bdd0be5b`, `af0999bb`; tinyfolder product | **scaffolded** — daemon auto-chain intentional **won’t** | 7 | 6 | 7 | little-tool CLI `expand` |
| 5 | `trace-memory` | Cross-task trace retrieve → inject strategies | `9a3763c7`; tinyfolder product | **matured** — durable `data-next` JSONL recall+record | 7 | 6 | 7 | little-tool CLI `recall`/`record` |
| 6 | `learning-loop-A` | Close Phase A learning loop (highest impact / lowest risk per research) | `75eb5004`, `6eaf665f`; tinyfolder product | **CLOSED** — proof + ring + `--aged-vault` + census + mid-band thicken + unified `night_id` | 9 | 5 | 6 | deepen dream/spark wiring (not new organ) |
| 7 | `pdu-reflect` | Prosecutor–Defender–Umpire Hegelian reflection | `190d23e6`, `c59bdcd9`, `f13f17b9`; how-could-we-blueprint-an-idea | **matured** — `--mode llm` opt-in + heuristic fallback | 6 | 5 | 8 | little-tool CLI `reflect` |
| 8 | `stigmergy-queue` | Ideas→Architecture→Build→QA folder pipeline | `a4949023`, `54aec32d`, `d1473f8e`, `a5e885a4`; du-hast-gesagt + sovereign factory | **matured** — frontier ingest+advance on WORK tree | 6 | 4 | 6 | little-tool CLI `status`/`ingest`/`advance` |
| 9 | `ipw-dashboard` | Intelligence-per-Watt IDE status + hot-swap | `d2446b2a`, `2d7b181e`, `afe212a4`; Obolus VS Codium research | **parked** — Obolus shipped; IDE extension separate track | 7 | 4 | 6 | VSCodium extension (separate from little-tools) |
| 10 | `phantom-drive` | Air-gapped USB sterile LLM + mountpoint watchdog | `4ccab818`, `62bed1b2`, `9ddb4fa0`; phantom-drive micros | **parked** — hardware / out of LTL scope | 5 | 3 | 9 | research / hardware product — out of little-tools scope |
| 11 | `cascading-compiler` | LLM-Compiler → Executable Wisdom wiki cascade | `f36d8361`, `21eab4a8`, `da4ee01f`; cascading honeypot sources + wiki entities | **CLOSED (docs)** — [CASCADING_COMPILER.md](../docs/CASCADING_COMPILER.md) | 8 | 4 | 7 | positioning + deepen wiki emit (not vault import) |
| 12 | `kos-positioning` | Knowledge OS / Sovereign Reasoning Engine narrative | `026f014e`, `92b1985a`, `32a14d01`, `35652172`; tinyfolder + core_insight | **CLOSED (docs)** — [KOS_POSITIONING.md](../docs/KOS_POSITIONING.md) | 9 | 9 | 5 | docs / MACHINE identity only |
