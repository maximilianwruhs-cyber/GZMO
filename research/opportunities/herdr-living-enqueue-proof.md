---
id: herdr-living-enqueue-proof
title: herdr pane-close → living takeaway enqueue proof
status: active
score: 21
uniqueness: 4
brain_profit: 4
credit_cost: 5
attention_cost: 3
usp_fit: 5
stack_ids: [s1]
created: 2026-07-20
updated: 2026-07-20
---

# herdr living enqueue proof

## Why rare

herdr close-ritual is GREEN as lab/contract; rare piece is **pane close → living CT101 distill enqueue** without workstation `--now` or dual writer.

## Brain profit

Operator shell work metabolizes into vault mass on the living host — distinct from takeaway *reminders*.

## Done when

One real (or demable SSH-mirrored) herdr session-close path enqueues living takeaway with `now_flag=false`; artifact proves living session/queue; herdr + brain-feed checks GREEN.
