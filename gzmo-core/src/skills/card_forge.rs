...
pub fn build_user_prompt(meta: &AttractorMeta, sel: &ForgeSelection) -> String {
    let mut lines = vec![
        format!(
            ...
            sel.rarity, capitalize_card_arg(sel.color), sel.card_type, sel.set_code
        ),
        format!(
            "Forge lens: {} — {}",
            sel.forge_mode.label(),
...
pub fn parse_card(raw: &str, fallback_type: &str) -> ParsedCard {
    ...
    let pt = line_value(raw, "PT:").filter(|p| {
        !p.eq_ignore_ascii_case("NONE") && !p.eq_ignore_ascii_case("N/A")
    });
    ParsedCard {
        name,
        cost,
