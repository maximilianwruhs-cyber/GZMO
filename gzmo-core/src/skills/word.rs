...
impl Skill for WordSkill {
    ...
    async fn execute(&self, ctx: SkillContext<'_>) -> Result<SkillOutput> {
        ...
        let mut recent_themes =
            themes_from_fingerprints(&recent_words, "the invented word");
        ...
        let (result_text, brief) = word_data.ok_or_else(|| {
            anyhow::anyhow!(
                "LLM offline, word failed quality gate, or repeated too many times"
            )
        })?;
...
mod tests {
    ...
    fn test_normalize_invented_word() {
        assert_eq!(normalize_fingerprint("KRAKTIC (KRAK-tik)"), "kraktickraktik");
