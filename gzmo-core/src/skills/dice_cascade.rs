...
pub fn tables() -> &'static CascadeTables {
    TABLES.get_or_init(|| {
        CascadeTables::parse(EMBEDDED_TOML).unwrap_or_else(|e| {
            panic!("invalid embedded data/dice_cascade.toml: {e}")
        })
    })
}
...
/// Drop skills whose mapped graph node still has unmastered prerequisites.
pub fn filter_skills_by_readiness(
    skills: &[String],
    skill_prereqs: &HashMap<String, String>,
    ...
    let filtered: Vec<String> = skills
        .iter()
        .filter(|skill| {
            let Some(node_id) = skill_prereqs.get(skill.as_str()) else {
                return true;
            };
            graph.unmastered_prerequisites(node_id, mastered).is_empty()
        })
        .cloned()
        .collect();
    filtered
...
pub fn plan_cascade(
    ...
    snap: &ChaosSnapshot,
    skills_dir: &Path,
    readiness: Option<(&PrerequisiteGraph, &[String])>,
) -> Option<CascadePlan> {
    if !cfg.enabled {
        return None;
    }
    ...
    let mut pool: Vec<String> = band
        ...
        .cloned()
        .collect();
    if let Some((graph, mastered)) = readiness {
        let gated = filter_skills_by_readiness(&pool, &tables().skill_prereqs, Some(graph), mastered);
        if !gated.is_empty() {
            pool = gated;
        }
    }
...
pub fn cascade_evidence_json(
    ...
    if !output.display.is_empty() {
        value["display_plain"] = serde_json::Value::String(
            crate::text_util::pi_skill_display(&output.display),
        );
    }
    value
}
...
mod tests {
    ...
    fn nat1_catastrophe_pool() {
        let cfg = DiceCascadeConfig::default();
        let plan = plan_cascade(&cfg, 20, 1, 0, 1, &snap(1), Path::new("/tmp/skills"), None).unwrap();
    ...
    #[test]
    fn readiness_gate_blocks_unmastered_prereqs() {
        use crate::pedagogy::graph::PrerequisiteGraph;
        let graph = PrerequisiteGraph {
            domain: "test".into(),
            nodes: vec![
                crate::pedagogy::graph::PrerequisiteNode {
                    id: "borrowing".into(),
                    title: "Borrowing".into(),
                    description: String::new(),
                    prerequisites: vec![],
                    bloom_level: String::new(),
                },
                crate::pedagogy::graph::PrerequisiteNode {
                    id: "async_rust".into(),
                    title: "Async Rust".into(),
                    description: String::new(),
                    prerequisites: vec!["borrowing".into()],
                    bloom_level: String::new(),
                },
            ],
        };
        let mut prereqs = HashMap::new();
        prereqs.insert("story".into(), "async_rust".into());
        let skills = vec!["quote".into(), "story".into()];

        let gated = filter_skills_by_readiness(&skills, &prereqs, None, &[]);
        assert_eq!(gated, skills);

        let gated = filter_skills_by_readiness(&skills, &prereqs, Some(&graph), &[]);
        assert_eq!(gated, skills);

        let gated = filter_skills_by_readiness(
            &skills,
            &prereqs,
            Some(&graph),
            &["shell_navigation".into()],
        );
        assert_eq!(gated, vec!["quote".to_string()]);
