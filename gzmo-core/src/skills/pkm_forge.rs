...
const ELEMENTS: &[&str] = &["fire", "water", "grass", "electric", "psychic", "darkness", "metal", "colorless"];
...
pub fn build_user_prompt(meta: &AttractorMeta, sel: &ForgeSelection) -> String {
    let mut lines = vec![
        format!(
            ...
            sel.rarity, capitalize_pkm_arg(&sel.element), sel.category, sel.set_code
        ),
        format!(
            "Forge lens: {} — {}",
            sel.forge_mode.label(),
...
fn split_ability(ability: &str) -> Option<(String, String)> {
    ...
    if parts.len() >= 2 {
        Some((parts[0].trim().to_string(), parts[1..].join("|").trim().to_string()))
    } else {
        None
    }
}
...
fn split_attack(attack: &str) -> Option<(String, String, String, String)> {
    ...
    if parts.len() == 3 {
        ...
        Some((parts[0].trim().to_string(), "".to_string(), parts[1].trim().to_string(), parts[2].trim().to_string()))
    } else if parts.len() >= 4 {
        // Name | Cost | Damage | Description
        Some((parts[0].trim().to_string(), parts[1].trim().to_string(), parts[2].trim().to_string(), parts[3..].join("|").trim().to_string()))
    } else {
        None
    }
}
...
fn render_pkm_frame(sel: &ForgeSelection, card: &ParsedPkm, ultra_or_secret: bool) -> String {
    ...
    if card.category == "Pokemon" {
        ...
        lines.push(format!(
            ...
            name_str, "", width = (41 - name_str.len()).max(1)
        ));
    } else {
    ...
    if card.category == "Pokemon" {
        ...
        let retreat_str = card.retreat.map(|r| r.to_string()).unwrap_or_else(|| "0".to_string());
        let weak_retreat = format!("Weakness: {}  Retreat: {}", weak_str, retreat_str);
        lines.push(format!(
            "{border}  ║{RESET}  {DIM}{weak_retreat}{RESET}"
        ));
    } else {
        lines.push(format!("{border}  ║{RESET}"));
    ...
    if !card.flavor.is_empty() {
        for part in wrap_rules_text(&card.flavor, 39) {
            lines.push(format!(
                "{border}  ║{RESET}  {DIM}{MAGENTA}{part}{RESET}"
            ));
        }
        lines.push(format!("{border}  ║{RESET}"));
    }
