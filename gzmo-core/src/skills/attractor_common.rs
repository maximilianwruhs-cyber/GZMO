...
impl AttractorMeta {
    pub fn from_input(input: AttractorPromptInput<'_>) -> Self {
        ...
        Self {
            ...
            cabinet_echo: input.snap.incubating_previews.first().cloned(),
            anti_repeat_hint,
        }
    }
    ...
    pub fn user_prompt(&self, seed_label: &str, max_chars: usize, extra_rules: &[&str]) -> String {
        ...
        lines.push("- Output ONLY the creative text, no title, no quotes, no markdown.".to_string());
...
mod tests {
    ...
    fn nonce_varies_by_serial() {
        assert_ne!(build_nonce("wit", 10, 1, 1, 0), build_nonce("wit", 10, 2, 1, 0));
    }
