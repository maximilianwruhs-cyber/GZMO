...
use super::pkm_forge::{
    build_pkm_evidence, build_selection, build_system_prompt, build_user_prompt,
    is_ultra_or_secret, load_pkmforge, parse_pkm, render_forge_display, resolve_pkm_category,
    validate_forged_pokemon, ChaosEventRef,
};
...
impl Skill for PkmSkill {
    ...
    async fn execute(&self, ctx: SkillContext<'_>) -> Result<SkillOutput> {
        ...
        if is_ultra_or_secret(&sel) {
            feedback.push(ChaosEvent::Custom {
                ...
                thought_seed: Some(ThoughtSeed {
                    ...
                    text: format!("ULTRA EX FORGE: {} — the attractor shivers", parsed.name),
                }),
            });
        }
        for ev in &feedback {
        ...
        let evidence =
            build_pkm_evidence(&parsed, &sel, &attractor, &display, &hash, &feedback_refs);
