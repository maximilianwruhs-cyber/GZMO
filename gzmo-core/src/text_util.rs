...
mod tests {
    ...
    #[test]
    fn pi_skill_display_keeps_box_chars() {
        let raw = "┌─ DICE ─┐\n│ roll 5 │\n└────────┘";
        let plain = pi_skill_display(raw);
        assert!(plain.contains('┌'));
        assert!(plain.contains("roll 5"));
    }

    #[test]
    fn plain_skill_display_drops_box_chars() {
        let raw = "┌─ DICE ─┐\n│ roll 5 │\n└────────┘";
        let plain = plain_skill_display(raw);
        assert!(!plain.contains('┌'));
        assert!(plain.contains("roll 5"));
    }
}
